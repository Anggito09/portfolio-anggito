(()=>{
  ...
  preview.innerHTML=`
    <div class="babyops-preview-head">
      ...
      <small>Selected BabyOps screens covering onboarding, Home, cry recording, recognition result, and guided solution.</small>
    </div>
  ...
  style.textContent=`
    ...
    #projects .babyops-preview-head small{flex-basis:100%;max-width:760px;font-size:.72rem;line-height:1.55;color:#8fa2bd}
    #projects .babyops-preview-grid{position:relative;z-index:1;display:grid;grid-template-columns:repeat(8,minmax(0,1fr));gap:14px}
    ...
    @media(max-width:1280px){#projects .babyops-preview-grid{grid-template-columns:repeat(8,minmax(150px,1fr));overflow-x:auto;scroll-snap-type:x mandatory;padding:2px 2px 8px;scrollbar-width:none}#projects .babyops-preview-grid::-webkit-scrollbar{display:none}#projects .babyops-screen{scroll-snap-align:start}#projects .babyops-screen img{height:300px}}
    @media(max-width:560px){#projects .babyops-preview{margin:18px 0;padding:14px;border-radius:17px}#projects .babyops-preview-head{margin-bottom:13px}#projects .babyops-preview-head strong{font-size:.92rem}#projects .babyops-preview-head small{font-size:.68rem}#projects .babyops-preview-grid{grid-template-columns:repeat(8,148px);gap:10px}#projects .babyops-screen{padding:8px 8px 7px;border-radius:15px}#projects .babyops-screen img{height:272px;border-radius:11px}#projects .babyops-screen figcaption{font-size:.64rem}}
