(()=>{
  const ensureStyle=()=>{
    ...
    style.textContent=`
        ...
        .handsight-enhanced .handsight-title{position:relative;display:inline-block;max-width:860px;background:linear-gradient(100deg,#fff 8%,#e0f2fe 30%,#67e8f9 50%,#c4b5fd 70%,#fff 92%);background-size:260% 100%;-webkit-background-clip:text;background-clip:text;color:transparent;animation:handsightTitleShine 5.5s linear infinite}
        .handsight-enhanced .handsight-title::after{content:"";position:absolute;left:0;right:0;bottom:-6px;height:3px;border-radius:999px;background:linear-gradient(90deg,#06b6d4,#8b5cf6,transparent);transform:scaleX(0);transform-origin:left;animation:handsightTitleLine 2.6s ease-in-out infinite}
        .handsight-enhanced .handsight-title:hover{filter:drop-shadow(0 0 16px rgba(6,182,214,.45))}
        ...
        @keyframes handsightTitleShine{0%{background-position:0% 50%}100%{background-position:260% 50%}}@keyframes handsightTitleLine{0%{transform:scaleX(0);opacity:.3}45%{transform:scaleX(1);opacity:1}70%{transform:scaleX(1);opacity:1}100%{transform:scaleX(0);opacity:0}}
