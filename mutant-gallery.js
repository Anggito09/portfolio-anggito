  ...
  const init=()=>{
    ...
    if(!document.getElementById('mutant-showcase-style')){
      ...
      style.textContent=`
        ...
        /* ===== TITLE ANIMATION — premium kinetic violet ===== */
        .mutant-card-enhanced .mutant-title{position:relative;display:inline-block;background:linear-gradient(100deg,#fff 6%,#ede9fe 22%,#c4b5fd 42%,#818cf8 62%,#a5b4fc 78%,#fff 92%);background-size:300% 100%;-webkit-background-clip:text;background-clip:text;color:transparent;animation:mutantTitleShine 4.2s linear infinite,mutantTitleFloat 3.8s ease-in-out infinite,mutantTitleIn .85s cubic-bezier(.2,.7,.2,1) both;filter:drop-shadow(0 0 14px rgba(167,139,250,.28))}
        .mutant-card-enhanced .mutant-title::after{content:"";position:absolute;left:0;right:0;bottom:-6px;height:3px;border-radius:999px;background:linear-gradient(90deg,#a78bfa,#818cf8,#60a5fa,transparent);transform:scaleX(0);transform-origin:left;animation:mutantTitleLine 2.4s ease-in-out infinite}
        .mutant-card-enhanced .mutant-title:hover{filter:drop-shadow(0 0 18px rgba(167,139,250,.6)) brightness(1.08)}
        /* ===== DESC — justify without hyphen ===== */
        .mutant-card-enhanced .mutant-desc{max-width:980px;margin:10px 0 6px!important;font-size:.95rem!important;line-height:1.78!important;color:#ddd6fe!important;text-align:justify!important;text-justify:inter-word!important;hyphens:none!important;-webkit-hyphens:none!important;word-break:normal!important;overflow-wrap:break-word!important;animation:mutantDescIn .7s .18s both}
        /* ===== HEADER BADGE ===== */
        ...
        @keyframes mutantTitleShine{0%{background-position:0% 50%}100%{background-position:300% 50%}}@keyframes mutantTitleLine{0%{transform:scaleX(0);opacity:.3}45%{transform:scaleX(1);opacity:1}70%{transform:scaleX(1);opacity:1}100%{transform:scaleX(0);opacity:0}}
        @keyframes mutantTitleFloat{0%,100%{transform:translateY(0)}50%{transform:translateY(-2px)}}@keyframes mutantTitleIn{from{opacity:0;transform:translateY(14px) scale(.98)}to{opacity:1;transform:none}}@keyframes mutantDescIn{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:none}}
